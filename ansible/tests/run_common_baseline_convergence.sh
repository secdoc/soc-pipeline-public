...
name="soc-iac-common-baseline-$RANDOM-$$"
...
if [[ -n "$apt_main" ]]; then
  ...
  "$engine" exec --env APT_MAIN="$apt_main" --env APT_SECURITY="$apt_security" \
    --env ANSIBLE_CORE_VERSION="$ansible_core_version" "$name" sh -ec '
    ...
    DEBIAN_FRONTEND=noninteractive apt-get -o Acquire::https::CaInfo=/usr/local/share/ca-certificates/ansible-test-root.crt install -y "ansible-core=$ANSIBLE_CORE_VERSION" ca-certificates >/dev/null
